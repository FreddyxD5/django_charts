...
def spend_per_customer_chart(request, year):
    ...
    for group in grouped_purchases:
        spend_per_customer_chart[months[group["month"]-1]] = round(group["average"], 2)
...
def payment_method_chart(request, year):
    purchases = Purchase.objects.filter(time__year=year)
    grouped_purchases = purchases.values("payment_method").annotate(
        count=Count("id")).values("payment_method", "count").order_by("payment_method")
